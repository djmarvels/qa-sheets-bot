import { GoogleSpreadsheet } from 'google-spreadsheet'
import { JWT } from 'google-auth-library';

const unique_apricot = {
    "google_spreadsheet_id": "1OTzoLLxq5Pv7zmpnQ37CwbNl5aGpjsgDNvBm0WsuYV8",
    "client_mail": "qa-nornikel-bot@unique-apricot-279722.iam.gserviceaccount.com",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
};

export const serviceAccountAuth = new JWT({
    email: unique_apricot.client_mail,
    key: unique_apricot.private_key,//s
    scopes: [
        'https://www.googleapis.com/auth/spreadsheets',
    ],
});
export const doc = new GoogleSpreadsheet(unique_apricot.google_spreadsheet_id, serviceAccountAuth);

export const spreadSheetInit = () => new Promise((resolve, reject) => {
    doc.loadInfo().then(data => resolve(data)).catch(error => reject(error));
});

export default () => {
    console.log('test');
};